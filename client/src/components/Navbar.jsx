...
const NavbarItem = ({ title, classProps }) => {
  return (
    <li className={`mx-4 currsor-pointer ${classProps}`}>
      {title}
    </li>
  );
}
  ...
  return (
    <nav className='w-full flex md:justify-center justify-between items-center p-4'>
      ...
      <div className='flex relative'>
        {toggleMenu ? <AiOutlineClose fonstSize={28} className='text-white md:hidden cursor-pointer' onClick={() => setToggleMenu(false)} /> : <HiMenuAlt4 fonstSize={28} className='text-white md:hidden cursor-pointer' onClick={() => setToggleMenu(true)} />}
        {toggleMenu && (
          <ul
